...
import { generarDiagnosticoPDF } from "@/lib/pdf-generator";
      ...
      {tab === "resumen" && (
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            {lastDiag && (
              <>
                <Link href={`/diagnosticos/${lastDiag.id}/presentacion-b`}>
            ...
            diagnosticos.map((d) => (
              <div
                ...
                onClick={() =>
                  router.push(
                    d.estado === "completo"
                      ? `/diagnosticos/${d.id}/presentacion-b`
                        ...
                        {act.diagnostico_id && (
                          <div className="shrink-0">
                            <Button
                              ...
                              onClick={() => {
                                if (isBalance) {
                                  // Navigate to vista-completa where BalancePDFTemplate is mounted
                                  // and DiagnosticoProvider loads real client data before capturing.
                                  const qp = clienteId ? `?clienteId=${encodeURIComponent(clienteId)}&autoDownload=true` : "?autoDownload=true";
                                  router.push(`/diagnosticos/${act.diagnostico_id}/completado${qp}`);
                                } else {
                                  void generarDiagnosticoPDF(nombre, { diagnosticoId: act.diagnostico_id!, token: getAccessToken() ?? undefined });
                                }
                              }}
                            >
                              Descargar
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            if (isBalance && doc.diagnostico_id) {
                              const qp = clienteId ? `?clienteId=${encodeURIComponent(clienteId)}&autoDownload=true` : "?autoDownload=true";
                              router.push(`/diagnosticos/${doc.diagnostico_id}/completado${qp}`);
                            } else {
